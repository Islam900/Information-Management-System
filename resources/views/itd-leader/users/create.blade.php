            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3>Yeni işçi</h3>
                        <a href="{{route('itd-leader.users.index')}}">
                ...
                <div class="card-body">
                    <form method="POST" action="{{route('itd-leader.users.store')}}">
                        @csrf
                        <div class="row">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#main_position_select').on("change", function () {
                ...
                {
                    $.ajax({
                        url:"{{ url('itd-leader/get-positions-by-management-board') }}",
                        type:"POST",
                        data:{
                            "_token":"{{csrf_token()}}",
                        },
            ...
            $('#department_select').on('change', function(e){
                ...
                $.ajax({
                    url:"{{ url('itd-leader/get-branches-by-department') }}",
                    type:"POST",
                    data:{
                        "_token":"{{csrf_token()}}",
            ...
            $('#branch_select').on('change', function(e){
                ...
                {
                    $.ajax({
                        url:"{{ url('itd-leader/get-positions-by-branch') }}",
                        type:"POST",
                        data:{
                            "_token":"{{csrf_token()}}",
                            "branch_id": branch_id
                ...
                {
                    $.ajax({
                        url:"{{ url('itd-leader/get-positions-by-null-department') }}",
                        type:"POST",
                        data:{
                            "_token":"{{csrf_token()}}",
                            "department_id": department_id,
